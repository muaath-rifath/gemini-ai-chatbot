...
const Gemini = GObject.registerClass(
  class Gemini extends PanelMenu.Button {
    ...
    _fetchSettings() {
      ...
      BACKGROUND_COLOR_HUMAN_MESSAGE = settings.get_string(
        "human-message-color"
      );
      BACKGROUND_COLOR_GEMINI_MESSAGE = settings.get_string(
    ...
    _init() {
      try {
        // Create the icon with proper system tray styling
        let icon = new St.Icon({
          icon_name: "system-run-symbolic", // Use a default symbolic icon first
          style_class: "system-status-icon",
        });

        // Add the icon to the panel button
        this.add_child(icon);
